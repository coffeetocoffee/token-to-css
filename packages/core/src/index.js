/**
 * `@token-to-css/core` — the compiler surface of `token-to-css`.
 *
 * This is the real, standalone npm package produced by the v11.5 split. It
 * contains zero plugin dependencies: connectors, the editor, the MCP server, the
 * relay, the adoption engine, and the `serve` HTTP layer live in the
 * batteries-included `token-to-css` meta-package (or their own opt-in packages).
 *
 * Connectors and plugins depend only on this public surface via
 * `registerPlugin` / `registerFunction` / `registerFormat` / `registerConnector`,
 * so a `@token-to-css/core` install never pulls in `serve`, `editor`, or any
 * connector file.
 */
...
export { deepMerge, mergeTokens } from "./merge.js";
export { mapToBarefoot, BAREFOOT_MAP } from "./presets/barefoot.js";
